    ...
    <div className="p-8">
        ...
        <div className="flex-1">
          {editingProjet ? (
            <input
              ...
              className="text-2xl font-bold text-[#0A0A0A] border-b-2 border-[#E6007E] bg-transparent focus:outline-none w-full"
            />
          ) : (
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-[#0A0A0A]">{projet.nom}</h1>
        ...
        <div className="flex gap-2">
          {!editingProjet ? (
            ...
            <>
              ...
              <button onClick={handleSaveProjet} disabled={loading} className="flex items-center gap-2 px-3 py-2 text-sm bg-[#E6007E] text-white rounded-xl hover:bg-[#C00069] transition-colors disabled:opacity-50">
          ...
          {editingProjet ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-[#E6007E]">{projet.progression}%</span>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-3">
                <div className="h-3 rounded-full bg-gradient-to-r from-[#E6007E] to-[#E6007E]" style={{ width: `${projet.progression}%` }} />
              </div>
            </>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-[#0A0A0A]">Tâches</h2>
          <button
            onClick={() => setShowNewTache(true)}
            className="flex items-center gap-2 px-3 py-2 text-sm bg-[#E6007E]/10 text-[#E6007E] rounded-xl hover:bg-[#E6007E]/20 transition-colors font-medium"
          >
        ...
        {showNewTache && (
          <form onSubmit={handleCreateTache} className="mb-4 p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-3">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button type="submit" disabled={loading} className="px-3 py-1.5 text-xs bg-[#E6007E] text-white rounded-xl hover:bg-[#C00069] transition-colors disabled:opacity-50">Créer</button>
            </div>
          </form>
        )}
              ...
              return (
                <div key={todo.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${isDone ? "bg-gray-50 border-gray-100 opacity-60" : "bg-violet-50/50 border-violet-100 hover:border-violet-200"}`}>
                  <button
                    ...
                    className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${isDone ? "bg-emerald-500 border-emerald-500 text-white" : "border-gray-300 hover:border-[#E6007E]"}`}
                  >
            ...
            filteredTaches.map((tache) => (
              <div key={tache.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${tache.statut === "termine" ? "bg-gray-50 border-gray-100 opacity-70" : "bg-white border-gray-100 hover:border-gray-200"}`}>
                <button
                  ...
                  className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${tache.statut === "termine" ? "bg-emerald-500 border-emerald-500 text-white" : "border-gray-300 hover:border-[#E6007E]"}`}
                >
                ...
                {editingTache === tache.id ? (
                  <div className="flex-1 grid grid-cols-4 gap-2">
                    <input value={editTacheForm.titre || ""} onChange={(e) => setEditTacheForm({ ...editTacheForm, titre: e.target.value })} className="col-span-2 px-2 py-1 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#E6007E]" />
                    <select value={editTacheForm.statut || ""} onChange={(e) => setEditTacheForm({ ...editTacheForm, statut: e.target.value as Tache["statut"] })} className="px-2 py-1 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#E6007E] bg-white">
                      <option value="a_faire">À faire</option>
                      <option value="en_cours">En cours</option>
                      <option value="termine">Terminé</option>
                    </select>
                    <select value={editTacheForm.priorite || ""} onChange={(e) => setEditTacheForm({ ...editTacheForm, priorite: e.target.value as Tache["priorite"] })} className="px-2 py-1 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#E6007E] bg-white">
                      <option value="haute">Haute</option>
                      <option value="moyenne">Moyenne</option>
                      <option value="basse">Basse</option>
                    </select>
